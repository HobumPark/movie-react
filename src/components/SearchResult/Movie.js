...
function Movie(props) {
    ...
    useEffect(() => {
        const selector1=('.movie'+props.movie_id)+" .user_rating"
        console.log(selector1)
        var rating = document.querySelector(selector1).innerText
        rating = parseFloat(rating)
        console.log(rating)    

        const selector2 = ('.movie' + props.movie_id) + " .rank_red"
    ...
    return (
        <div id='movie' className={'movie'+props.movie_id}>
            <div id='movie-left'>
                <a href={`/movie/view/main?id=${props.movie_id}&menu=1`}>
                    <img src={`/images/cover/movie${props.movie_id}.jpg`} alt='커버이미지'/>
                </a>
            </div>
            <div id='movie-right'>
                <div id='movie-title'>
